    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
                ...
                for (int i = 0; i < NumberOfPanels; i++)
                {
                    switch (movementAxis)
                    {
                        case MovementAxis.Horizontal:
                            ...
                            //Panels[i].gameObject.SetActive(Mathf.Abs(GetDisplacementFromCenter(i).x) <= 50);
                            break;
                        case MovementAxis.Vertical:
                            Panels[i].gameObject.SetActive(Mathf.Abs(GetDisplacementFromCenter(i).y) <= Viewport.rect.height);
                            //Panels[i].gameObject.SetActive(Mathf.Abs(GetDisplacementFromCenter(i).y) <= 50);
        ...
        public int GetObjectIndex(GameObject gameObject)
        {
            if (Panels != null)
            {
                ...
                {
                    if (Panels[i].gameObject == gameObject)
                    {
                        return i;
                    }
                }
        ...
        {
            if (GetObjectIndex(panel) != NoMatchIdx)
            {
                Debug.LogError("<b>[SimpleScrollSnap]</b> GameObject " + panel.name + " is already in the Scroll Snapper");
                return;
            }
        ...
        {
            int indexOfGameObject = GetObjectIndex(gameObject);

            if (indexOfGameObject != NoMatchIdx)
            {
                Remove(indexOfGameObject);
            }
        }
